...
const tabMessages = {
  "Semua":      { icon: <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="1.8" strokeLinecap="round"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>, title: "Belum Ada Pesanan", desc: "Kamu belum pernah melakukan pembelian. Yuk mulai belanja!", showBtn: true },
  "Diproses":   { icon: <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="1.8" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>, title: "Tidak Ada Pesanan Diproses", desc: "Saat ini tidak ada pesanan yang sedang diproses.", showBtn: false },
  "Selesai":    { icon: <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="1.8" strokeLinecap="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>, title: "Belum Ada Pesanan Selesai", desc: "Pesanan yang sudah selesai akan muncul di sini.", showBtn: false },
  "Dibatalkan": { icon: <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="1.8" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>, title: "Tidak Ada Pesanan Dibatalkan", desc: "Bagus! Kamu tidak memiliki pesanan yang dibatalkan.", showBtn: false },
};
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-7 max-w-sm w-full shadow-2xl">
        ...
        <p className="text-sm text-gray-500 text-center mb-1">Pesanan <strong>{order.orderNumber}</strong> akan dibatalkan.</p>
        <p className="text-xs text-gray-400 text-center mb-6">Pembatalan hanya bisa dilakukan dalam 1x24 jam setelah pesanan dibuat.</p>
        <div className="flex gap-3">
          <button onClick={onClose}
            className="flex-1 py-2.5 border border-gray-200 rounded-xl text-sm font-semibold text-gray-600 cursor-pointer hover:bg-gray-50 transition-colors bg-white">
            Kembali
          </button>
          <button onClick={onConfirm}
            className="flex-1 py-2.5 bg-red-500 text-white rounded-xl text-sm font-bold border-none cursor-pointer hover:bg-red-600 transition-colors">
            Ya, Batalkan
          </button>
        </div>
      </div>
...
export default function Orders({ onNavigate }) {
  ...
  const { orders, cancelOrder, canCancel } = useOrder();

  const filtered = activeTab === "Semua" ? orders : orders.filter(o => o.status === activeTab);

  const handleConfirmCancel = () => {
    cancelOrder(cancelTarget.orderNumber);
            ...
            <div className="flex flex-col gap-4">
              {filtered.map((order, idx) => (
                <div key={idx} className="border border-gray-200 rounded-xl p-5 hover:shadow-md transition-shadow">
                  ...
                  {/* Info kelas & jurusan */}
                  {(order.kelas || order.jurusan) && (
                    <div className="flex items-center gap-1.5 mb-3 text-xs text-gray-500">
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" strokeLinecap="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg>
                      {order.kelas} · {order.jurusan}
                    </div>
                  )}
                  ...
                  <div className="border-t border-gray-100 pt-3 flex justify-between items-center flex-wrap gap-2">
                    ...
                    {order.status === "Diproses" && !canCancel(order) && (
                      <span className="text-xs text-gray-400">Melewati batas pembatalan</span>
                    )}
                  </div>
                </div>
              ))}
